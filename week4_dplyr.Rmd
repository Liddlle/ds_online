...
```{r exe13-hint, echo = FALSE}
```

```{r exe13-solution, echo = FALSE}
colleges = filter(colleges, Grad.Rate<50)

```

```{r exe13-check, echo = FALSE}
...
test_4 <- check_argument("filter(.data = grab_this)", match_data_frame(colleges), "use colleges as the data argument in filter()")
test_5 <- find_statement("Grad.Rate", "use `Grad.Rate variable`")
test_6 <- find_statement("< 50", "use `< 50`") #вроде не чувствителен к пробелу, но перепроверить
...
```{r exe13-setup, echo = FALSE}

```

```{r exe13, exercise = TRUE, exercise.setup = "exe13-setup", exercise.eval = FALSE}
