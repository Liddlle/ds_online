...
test_4 <- find_statement("flData") 
test_6 <- find_call("coord_flip()", "didn't use coord_flip()")
...
ggplot() + geom_histogram(data = flData, aes(x = hour)) 
...
### Выбраем подмножество строк с filter()
...
### Упорядочить значения с arrange()
...
```{r quiz1, echo = FALSE}
quiz(caption = "",
     question("Какая функция используется для создания новых переменных:", 
            answer("summarise()"), 
           answer("group_by()"),
...
test_1 <- find_call("filter(whatever, whatever)", "didn't use filter() function")
test_2 <- find_statement("colleges", "didn't use `colleges` dataset") 
...
test_1 <- find_call("mutate()", "didn't use mutate() function")
test_2 <- find_statement("colleges", "didn't use `colleges` dataset") 
...
test_1 <- find_call("select()", "didn't use select() function")
test_2 <- find_statement("colleges", "didn't use `colleges` dataset") 
...
colleges %>% group_by(Private) %>% dplyr::summarise(mean_acceptance = mean(acceptance_rate),
...
test_3 <- find_call("group_by(.data = whatever, Private)", "break down the result by `cyl`")
test_4 <- find_call("summarise()", "use summarise() after grouping")
test_5 <- find_call("mean(acceptance_rate)", "calculate the mean of acceptance_rate for each group")
test_6 <- find_call("mean(enrolled_rate)", "calculate the mean of enrolled_rate for each group")
